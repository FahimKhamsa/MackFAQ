...
interface IBotCreatePayload extends CreateBotDTO { favicon_image_file: { data: Buffer, fileName: string }, background_image_file: { data: Buffer, fileName: string }, creator_id: number } 
...
export class BotsService {
    ...
    public saveFile(fileData: Buffer, fileExtensionOrName: string) {
        const fileExtension = fileExtensionOrName.includes('.') ?  /(?:\.([^.]+))?$/.exec(fileExtensionOrName)[1] : fileExtensionOrName;
        const dir = this.configService.get<string>('PUBLIC_FILES_STORAGE', 'storage/');

        const fileName = crypto.randomBytes(10).toString('base64url') + '-' + crypto.createHash('sha1').update(new Date().getTime().toString()).digest('base64url') + '.' + fileExtension;
    ...
    public async createBot(payload: IBotCreatePayload) {
        if (payload.background_image_file) {
            payload.background_image_path = this.saveFile(payload.background_image_file.data, payload.background_image_file.fileName).fileName;
            delete payload.background_image_file;
        }
        
        if (payload.favicon_image_file) {
            payload.favicon_path = this.saveFile(payload.favicon_image_file.data, payload.favicon_image_file.fileName).fileName;
            delete payload.favicon_image_file;
        }

        return await this.botModel.create(payload).then((bot: BotModel) => bot.dataValues);
    }
