...
export class SOPService {
    ...
    async uploadSOPDocument(file: { content: Buffer, name: string, mimetype: string }, config: { title: string, category: string, uploaded_by: number }) {
        // Validate file type
        ...
        if (!supportedMimes.includes(file.mimetype)) {
            throw new BadRequestException("SOP documents must be PDF, CSV, XLS, XLSX, TXT, JPG, PNG, or GIF files");
        }
        ...
        try {
            ...
            throw new BadRequestException(`Failed to process SOP file: ${error.message}`);
        }
        ...
        sopDocument.page_count = this.estimatePageCount(file.content, file.mimetype);
    ...
    private async processSOPFile(file: { content: Buffer, name: string, mimetype: string }) {
    ...
    async getAllSOPDocuments(filters?: { category?: string, is_active?: boolean }) {
    ...
    async updateSOPDocument(id: number, updates: { title?: string, category?: string, is_active?: boolean }) {
        ...
        if (updates.is_active !== undefined) sopDocument.is_active = updates.is_active;
    ...
    async bulkUploadSOPs(files: Array<{ content: Buffer, name: string, mimetype: string }>, config: { uploaded_by: number }) {
        const results = [];
        const errors = [];
    ...
    private detectSOPCategory(filename: string): string {
        ...
        if (name.includes('quality') || name.includes('qa') || name.includes('qc')) return 'quality';
        if (name.includes('hr') || name.includes('human') || name.includes('employee')) return 'hr';
        if (name.includes('finance') || name.includes('accounting') || name.includes('budget')) return 'finance';
        if (name.includes('it') || name.includes('tech') || name.includes('system')) return 'it';
        if (name.includes('operation') || name.includes('process') || name.includes('procedure')) return 'operations';
        if (name.includes('compliance') || name.includes('audit') || name.includes('regulation')) return 'compliance';
        if (name.includes('training') || name.includes('education') || name.includes('learning')) return 'training';
