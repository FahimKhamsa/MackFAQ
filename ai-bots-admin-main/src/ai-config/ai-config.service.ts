...
export class AIConfigService {
  ...
  async updateSystemPrompt(
    ...
    if (config.prompt_locked) {
      throw new BadRequestException(
        'System prompt is locked and cannot be modified',
      );
    }
  ...
  async updateSelectedModel(
    ...
    if (!config.available_models.includes(model)) {
      throw new BadRequestException(
        `Model ${model} is not available. Available models: ${config.available_models.join(
          ', ',
        )}`,
      );
    }
  ...
  async updateAPIKeys(
    userId: number,
    keys: { openai_api_key?: string; openrouter_api_key?: string },
  ): Promise<AIConfiguration> {
    const config = await this.getUserConfiguration(userId);
