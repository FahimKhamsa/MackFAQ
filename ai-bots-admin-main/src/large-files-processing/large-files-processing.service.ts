...
export class LargeFilesProcessingService implements OnModuleInit {
  ...
  public async processWithConversationChain(
    prompt: string,
    conversationData: IMessage[] = [],
    docsData: string[] = [
      `The President said about Justice Breyer that he is cool boy. Everyone said, that President's words are false`,
    ],
    config: {
    ...
    return await this.trainingLoaderService.processWithConversationChainOnly(
      prompt,
      conversationDataCompiled,
      config,
    );
  }

  private async privateApiProcessor(
    input: string,
    docsIds: string[],
    conversationData: IMessage[],
      ...
      this.httpService
        ...
        .pipe(
          map(
            (e) =>
              (e?.data?.data &&
                e?.data?.data?.map((v) => v?.document?.doc_id ?? null)) ||
              [],
          ),
        ),
    );
    ...
    const messages = conversationData.map((r) => ({
      content: r.message,
      role: msgTypeToPAPI[r.type],
    }));
    ...
    if (chatResult) {
      const sources = chatResult.sources
        .map(
          (source) =>
            `File: ${source.document.doc_metadata.file_name}. Page: ${
              source.document.doc_metadata.page_label ?? 'Not found'
            }`,
        )
  ...
  private formatChatHistory(chagHistory: IMessage[]) {
    ...
    for (const i in chagHistory) {
      if (
        !chagHistory[i].previousMessageId ||
        !(chagHistory[i].previousMessageId in messagesDict)
      ) {
        ...
        messagesDict[chagHistory[i].previousMessageId].push(
          chagHistory[i].message,
        );
      }
    }
  ...
  public async getAnswerFromApi(
    question: string,
    chagHistory: IMessage[],
    bot_id: string,
    promptPrefix: string = null,
  ): Promise<{ text: string; sources: { text: string }[] }> {
    const messagesHistory = this.formatChatHistory(chagHistory);

    this.logger.log({ messagesHistory });

    const response = await this.trainingLoaderService.getAnswer(
      question,
      messagesHistory,
      bot_id,
      promptPrefix,
    ...
    const result = {
      ...
      sources: response.sourceDocuments.map((v) => ({
        text: v.metadata.source as string,
      })) as { text: string }[],
    };
  ...
  public async getDocsIds(
    ...
    return savedLearning
      .filter((r) => !docsIds || docsIds.includes(r.id))
      .map((r) => r.docs_ids_from_provider)
  ...
  public async getDocs(config: { project_id?: number; bot_id?: number }) {
    ...
    return (await this.getAllConnectionsByProject(config.project_id)).map(
      (connection) => connection.learning_session,
    );
  }
  ...
  async legacyProjectsConnections() {
    ...
    for (const file of files) {
      ...
      await this.learningSessionProjectConnection.findOrCreate({
        paranoid: false,
        where: { project_id: file.project_id, learning_session_id: file.id },
      });
    }
  }
  ...
  private async learnPrivateApi(file: {
    content: Buffer;
    name: string;
    mimetype: string;
  }) {
    const supportedMimes = [
      ...
      'image/jpeg',
      'image/png',
      'image/gif', // Screenshots for compliance
    ];

    if (!supportedMimes.includes(file.mimetype)) {
      throw new BadRequestException(
        'You can upload .PDF, .CSV, .XLS, .XLSX, .TXT, .JPG, .PNG, or .GIF files only',
      );
    }
  ...
  private detectFileCategory(filename: string, mimetype: string): string {
    ...
    if (
      mimetype === 'text/plain' &&
      (name.includes('email') || name.includes('msg'))
    )
      return 'email';
    if (
      mimetype === 'text/plain' &&
      (name.includes('transcript') || name.includes('meeting'))
    )
      return 'transcript';
