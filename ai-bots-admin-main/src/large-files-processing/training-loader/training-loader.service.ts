...
export class TrainingLoaderService {
  ...
  constructor(
    @Inject('LARGE_FILES_PROCESSING_API_CONFIG')
    private largeFilesProcessingConfig: ILargeFilesProcessingApiConfig,
  ) {}
  ...
  public async processWithConversationChainOnly(
    message: string,
    messagesHistory: { question: string; answer: string }[],
    config: {
    ...
    const model = new ChatOpenAI({
      ...
      openAIApiKey: this.largeFilesProcessingConfig.OPEN_AI_KEY,
    });
    ...
    const chain = new LLMChain({
      ...
      prompt: new PromptTemplate({
        inputVariables: ['chat_history', 'human_message'],
        template: `${escapeBraces(
          config.prompt,
        )}\n\nChat History:\n{chat_history}\n\n${
          config.humanPrefix
        }:\n{human_message}\n\n${config.aiPrefix}:`,
  ...
  public async processWithConversationChain(
    ...
    if (!config.prompt) {
      config.prompt =
        'Your answer must be like a standart human message, be interesting';
    }
...
${config.prompt ? '7. ' + escapeBraces(config.prompt) : ''}.\n` + // If you don't know the answer, just say that you don't know, don't try to make up an answer.
  ...
  public async loadTrainingFiles(content: string, PINECONE_NAME_SPACE: string) {
    try {
      ...
      const index = (await this.getPineconeClient()).Index(
        this.largeFilesProcessingConfig.PINECONE_INDEX_NAME,
  ...
  public async getAnswer(
    ...
    try {
      ...
      const chain = makeChain(
        vectorStore,
        this.largeFilesProcessingConfig.OPEN_AI_KEY,
        promptPrefix,
        { message_as_continuation: chat_history.length >= 1 },
      );
