...
import { LLMChain, loadQAChain, ChatVectorDBQAChain, ConversationalRetrievalQAChain } from 'langchain/chains';
...
export const makeChain = (vectorstore: PineconeStore, apiKey: string, promptPrefix: string = null, config: { message_as_continuation?: boolean } = {}) => {
  ...
  const QA_PROMPT =
    PromptTemplate.fromTemplate(`${promptPrefix}.
...
Helpful answer in markdown:`);
  ...
  const docChain = loadQAChain(
    ...
    new OpenAI({ temperature: 0, modelName: 'gpt-3.5-turbo', openAIApiKey: apiKey, }),
    {
      type: "stuff",
      prompt: QA_PROMPT,
    },
