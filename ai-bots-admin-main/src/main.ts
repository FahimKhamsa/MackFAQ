...
function registerHelpers(hbs) {
  ...
  hbs.registerHelper('selectFromList', function (matchKey, matchValue, options, key) {
    return options.filter(v => v[matchKey] === matchValue)?.[0]?.[key];
  });
  hbs.registerHelper('paginationFooter', function (payload: { totalPages: number, page: number }, url: string) {
    const prev = payload.page >= 1 ? `<a  href="/${url + '?page=' + (payload.page - 1).toString()}" >Prev</a>` : '';
    const next = payload.page + 1 < payload.totalPages ? `<a  href="/${url + '?page=' + (payload.page + 1).toString()}" >Next</a>` : '';
    return prev + next;
  });
  hbs.registerHelper('nav-panel', function (current_page, options) {
    return options.fn(this).replace(
      new RegExp('nav-item-' + current_page + '', 'g'),
      '$& active');
  });
