import { Body, Controller, Delete, Get, HttpException, HttpStatus, Post, Put, Query, Req, Res, UploadedFiles, UseGuards, UseInterceptors } from '@nestjs/common';
...
import { IConversationId, IConversationIds, ICreateConversationDTO, IGetCompleteDTO, IProjectIdentification, IRecompleteDTO } from './dto/get-complete.dto';
...
export class ApiController {
    constructor(private apiService: ApiService, private botService: BotsService) {
    ...
    public async recomplete(@Query() query: IRecompleteDTO) {
        return {
            data: await this.apiService.deepFaq(null, query.conversationId, { project_id: null, bot_id: null, currentUserId: 0 }, null, query.messageId),
            // data: await this.apiService.getAnswer(query.prompt, query.conversationId, { ...query, translate_to_language: query.lang || null }),
        };
    }
    ...
    public async getDeepFaqComplete(@Query() query: IGetCompleteDTO) {
        return {
            data: await this.apiService.deepFaq(query.prompt, query.conversationId, { project_id: query.project_id, bot_id: query.bot_id, currentUserId: 0 }),
        };
    }
    ...
    public async conversationsHistory(@Query() query: IConversationIds) {
        return {
            data: await this.apiService.getConversationsHistory(query.conversationIds),
        };
    }

    @Get('project-history-compiled')
    @UseGuards(JwtAuthGuard)
    public async projectHistoryCompiled(@Query() query: IProjectIdentification, @Res() res: Response) {
        if (!query.bot_id || !query.project_id) {
            res.setHeader('Content-Type', 'text/plain').send("Please, set project_id and bot_id get parameters").status(400);
            return;
        }

        const conversationsList = Object.values(await this.apiService.getConversationsHistory(query)).sort((a, b) => {
            ...
            return Number(new Date(b.messages[b.messages.length - 1].createdAt)) - Number(new Date(a.messages[a.messages.length - 1].createdAt));
        });
        ...
        for (const chat of conversationsList) {
            ...
            for (const message of chat.messages) {
                ...
                if (message.createdAt) {
                    ...
                    dateStr = `${year}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')} ${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
                }

                text += `[${message.type === MessageTypes.AI_MESSAGE ? 'ROBOT' : 'CLIENT'} | ${dateStr}]\n${message.message}\n\n`
            }
    ...
    public async createConversation(@Body() body: ICreateConversationDTO) {
        return {
            data: await this.apiService.createConversation(body.conversationId, body.messages, false, body),
        };
    }
    ...
    public async appendConversation(@Body() body: ICreateConversationDTO) {
        return {
            data: await this.apiService.createConversation(body.conversationId, body.messages, true, body),
        };
    }
    ...
    @Put('file-connection')
    // @UseGuards(JwtAuthGuard) // Temporarily disabled for RAG testing
    public async connectFile(@Query('bot_id') bot_id: number, @Body() body: { project_id: number, learning_session_id: number, status: boolean }) {
        await this.apiService.connectLearnedFileToProject({ project_id: body.project_id, learning_session_id: body.learning_session_id, bot_id }, body.status);
        return {
            status: true,
        };
    }
    ...
    @Get('my-docs')
    // @UseGuards(JwtAuthGuard) // Temporarily disabled for RAG testing
    public async myConnections(@Query('bot_id') bot_id: number, @Query('project_id') project_id: number) {
        return {
            status: true,
    ...
    public async trainModel(@UploadedFiles() files, @Req() req: Request, @Query('bot_id') bot_id: number, @Body() body: { raw?: string }, @Query('project_id') project_id: number = null) {
        ...
        await this.apiService.trainWithFile({ content: file.buffer, name: file.originalname, mimetype: file.mimetype }, { bot_id, project_id });
        ...
        return {
            status: true,
            data: await this.apiService.trainModel(files?.file?.[0]?.buffer || inputText, { bot_id: +bot_id, project_id: project_id && project_id, mode: files?.file?.[0]?.buffer ? ( files?.file?.[0]?.originalname?.split('.').slice(-1)[0] ) : 'raw-lines' })
        };
    }

    @Delete('train')
    @UseGuards(JwtAuthGuard)
    async deleteTrain(@Query('bot_id') bot_id: number, @Query('project_id') project_id: number = null) {
        return {
            status: true,
    ...
    // @UseGuards(JwtAuthGuard) // Temporarily disabled for RAG testing
    async updateBotPrompt(@Body() body: { prompt_answer_pre_prefix: string, id: number, prompt_prefix: string }) {
        return {
            status: true,
