...
import { copyFile, existsSync, openSync, readFileSync, unlinkSync, writeFileSync } from 'fs';
import { GptapiService } from 'src/gptapi/gptapi.service';
import * as crypto from 'crypto';
import { BotsService } from 'src/bots/bots.service';
import { MessagesService } from 'src/messages/messages.service';
import { parse } from 'csv-parse';
import { IConfig, ITrainingInput, LocalIntentsResponsesStorageService } from 'src/local-intents-responses-storage/local-intents-responses-storage.service';
...
import { ICreateConversationDTO, IMessage, IProjectIdentification } from './dto/get-complete.dto';
...
export class ApiService {
    ...
    constructor(
        ...
        this._assistantLabel = this.configService.get<string>('_assistantLabel', '_assistantLabel');
        this._userLabel = this.configService.get<string>('_userLabel', '_userLabel');
        this._stopString = this.configService.get<string>('_stopString', '<|im_end|>');

        this._promptPrefix = '';
    }

    private newMessageId() {
        return crypto.randomBytes(5).toString('base64url') + '-' + Buffer.concat([Buffer.from(longToByteArray(+new Date())), crypto.randomBytes(5)]).toString('base64url');
    }
    ...
    private async GPTMatch(userPrompt: string, intents: {
        id: number;
        text: string;
    }[], maxMathches = 1, config: any) {
        intents = [...intents];
        const intentsById = Object.fromEntries(intents.map(({ id, ...v }) => [id, { ...v, id }]));
        const result = [];
        ...
        while (intents.length && (!promptResult || intents.length - searched > 0)) {
            try {
                promptResult = await this.formatPromptForIntentsMatcher(userPrompt, intents.slice(0, intents.length - searched), config);
                let choosedId = null;

                if (versionGPT4) {
                    // console.log(this.translateToGPT4(promptResult.promptBody));
                    const answer = await this.gptapiService.getComplete4(this.translateToGPT4(promptResult.promptBody));
    ...
    private async processLocalWithGPT(userPrompt: string, config: any) {
        ...
        const intents = await this.localIntentsResponsesStorageService.getAllIntents({ bot_id: config.bot_id, currentUserId: null, project_id: config.project_id });

        if (intents.length === 0) {
            return null;
        }

        const matched1 = await this.GPTMatch(userPrompt, intents.map(v => v.dataValues), 10, { ...config, messageType: 1 });
        const logForFirstStage = matched1.map(v => '--------\n\n' + v.id + ': ' + v.text + '\n\n--------').join('\n');
        ...
        const matched2 = matched1.length === 1 ? matched1 : await this.GPTMatch(userPrompt, matched1, 1, { ...config, messageType: 2 });
        const logForSecondStage = matched2.map(v => v.id + ': ' + v.text).join('\n');
        ...
        const response = await this.localIntentsResponsesStorageService.getResponseByIntentId(matched2[0].id);
        result = {
    ...
    private async processFaQWithGPT(userPrompt: string, chatHistory: IMessage[], config: any) {
        const _chatHistory = chatHistory.slice(0, -1);
        const result = await this.largeFilesProcessingService.getAnswerFromApi(userPrompt, _chatHistory, config.bot_id + '-' + config.project_id, config._promptPrefix);
    ...
    private isConversationInProcess(conversationId: string) {
        return this._conversationsInProcessData[conversationId] && this._conversationsInProcessData[conversationId].isProcessing;
    }

    private startProcessingConversation(conversationId: string, abortController: AbortController) {
    ...
    public async deepFaq(inputPrompt: string, conversationId: string = null, config: IConfig, conversationName?: string, regenerateAnswerFrom: string = null, messageTs: string = null, forceDisableDocsData = false, filesToUse = null) {
        if (!conversationId) {
            conversationId = crypto.createHash('md5').update(crypto.randomBytes(100)).update(Date.now().toString()).digest('base64url')
        ...
        if (config && config.project_id) {
            const project = await this.localIntentsResponsesStorageService.getProjectById(config.project_id);
            if (project) {
        ...
        const docsInput = await this.localIntentsResponsesStorageService.getRawData(config);
        const compiledDoc = await this.largeFilesProcessingService.createLearningInput(docsInput.map(v => ({ questions: v.questions.map(r => r.text), answer: v.answer.text })), config.bot_id + '-' + config.project_id);
        ...
        if (forceDisableDocsData === false) {
            docs = await this.largeFilesProcessingService.getDocsIds(config, filesToUse);
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            resolve(this.largeFilesProcessingService.processWithConversationChain(inputPrompt, promptBody, docs, { abortController: abortController, aiPrefix: aiPrefix, humanPrefix: humanPrefix, prompt: promptPrefix })
                ...
                .then((textAnswer) => {
                    ...
                    return this.savePrompt(promptBody, conversationId, { ...config, name: conversationName ?? null})
                        .then(() => {
                            this.stopProcessingConversation(conversationId);
    ...
    public async createConversation(conversationId: string = null, messagesToSave: IMessage[], append = false, config?: ICreateConversationDTO) {
        if (!conversationId) {
            // conversationId = crypto.randomBytes(20).toString('base64url') + crypto.randomUUID() + '-' + Buffer.from((+new Date()).toString()).toString('base64url');
            conversationId = crypto.createHash('md5').update(crypto.randomBytes(100)).update(Date.now().toString()).digest('base64url')
        }
        ...
        const getMessageList = async () => {
            if (append) {
                const history = (await this.getConversationHistory(conversationId)).messages;
        ...
        return new Promise((resolve, reject) => {
            ...
            resolve(
                getMessageList()
                    .then(messages => this.savePrompt(messages, conversationId, { name: config?.name ?? null, project_id: config.project_id ?? null, bot_id: config.bot_id ?? null }))
                    .then(() => {
                        this.stopProcessingConversation(conversationId);
                        return {
                            conversationId
    ...
    public async getAnswer(userPrompt: string, conversationId: string = null, params?: { bot_id: number, project_id?: number, translate_to_language?: string }) {
        if (conversationId) {
            this.validateConversationId(conversationId);
        }
        ...
        if (params && params.bot_id) {
            ...
            if (bot) {
                ...
                project = await this.localIntentsResponsesStorageService.getProjectById(params.project_id);

                config._userLabel = bot.user_name_label || this._userLabel;
                config._assistantLabel = bot.bot_name_label || this._assistantLabel;
                config._promptPrefix = (bot.prompt_prefix || this._promptPrefix) + '\n' + (project?.prompt_prefix || '');
            ...
            if (params.translate_to_language && params.translate_to_language in langs) {
        ...
        if (conversationId) {
            if (this._conversationsInProcess[conversationId]) {
                throw new HttpException('This conversation is in processing now!', HttpStatus.CONFLICT);
            }
        ...
        if (project && project.use_deep_faq) {
            try {
                processPromptData = await this.formatPrompt(userPrompt, conversationId, { ...config, GPT4: false });
                this.logger.log({ promptBody: processPromptData.promptBody });
                textAnswer = await this.processFaQWithGPT(userPrompt, processPromptData.promptBody, config);
        ...
        if (!textAnswer) {
            try {
                processPromptData = await this.formatPrompt(userPrompt, conversationId, config);
                const answer = await this.gptapiService.getComplete4(this.translateToGPT4(processPromptData.promptBody));
        ...
        if (conversationId && !this._conversationsInProcess[conversationId]) {
            throw new HttpException('This conversation is not in processing more!', HttpStatus.CONFLICT);
        }
        ...
        if (textAnswer && processPromptData) {
            processPromptData.promptBody.push({
                type: MessageTypes.AI_MESSAGE,
                message: textAnswer,
                previousMessageId: processPromptData.promptBody.length > 0 ? processPromptData.promptBody[processPromptData.promptBody.length - 1].messageId : null,
                messageId: this.newMessageId(),
            });

            this.savePrompt(processPromptData.promptBody, processPromptData.conversationId, config);

            const userMessage = await this.messagesService.createNewMessage({
                unique_id: processPromptData.promptBody[processPromptData.promptBody.length - 2].messageId,
                type: processPromptData.promptBody[processPromptData.promptBody.length - 2].type,
                text: processPromptData.promptBody[processPromptData.promptBody.length - 2].message,
            ...
            await this.messagesService.createNewMessage({
                unique_id: processPromptData.promptBody[processPromptData.promptBody.length - 1].messageId,
                type: processPromptData.promptBody[processPromptData.promptBody.length - 1].type,
                text: processPromptData.promptBody[processPromptData.promptBody.length - 1].message,
        ...
        if (textAnswer && (config._answerPrePromptPrefix || config.translate_to_language && false)) {
            ...
            try {
                const _processPromptData = await this.formatPrompt(textAnswer, null, config);
                const answer = await this.gptapiService.getComplete4(this.translateToGPT4(_processPromptData.promptBody));
        ...
        return {
            answer: textAnswer, conversationId: processPromptData?.conversationId, resultData
        };
    }
    ...
    public async defaultProject(bot_id: number) {
        const project = await this.localIntentsResponsesStorageService.defaultProject(bot_id);
        return project;
    }

    public async deleteAllIntents(bot_id: number, project_id: number = null) {
        return await this.localIntentsResponsesStorageService.clearProjectIntents(bot_id, project_id ?? (await this.defaultProject(bot_id)).id);
    }

    public async trainWithFile(file: { name: string, content: Buffer, mimetype: string }, params: { bot_id: number, project_id?: number}) {
        if (params.project_id) {
            const project = await this.localIntentsResponsesStorageService.getProjectById(params.project_id);
            if (!project || project.bot_id !== params.bot_id) {
                throw new BadRequestException("Project is not found");
            }
        }
        await this.largeFilesProcessingService.learnProviderWithFile("private-api", file, { ...params, project_id: params.project_id ?? null, creator_id: 0 });
    }

    public async connectLearnedFileToProject(payload: { project_id: number, learning_session_id: number, bot_id?: number }, is_connected: boolean) {
        const project = await this.localIntentsResponsesStorageService.getProjectById(payload.project_id);
        const file = await this.largeFilesProcessingService.getLearningSessionById(payload.learning_session_id);
        if (!project || !file || project.bot_id !== file.bot_id || payload.bot_id && payload.bot_id !== project.bot_id) {
            throw new BadRequestException("Project and/or file are not found");
        }
        if (is_connected) {
            return await this.largeFilesProcessingService.connectLearningSeesionToProject(payload);
        }
        return await this.largeFilesProcessingService.disconnectLearningSeesionToProject(payload);
    }
    ...
    public async trainModel(trainCsv: Buffer | string, params: { bot_id: number, project_id?: number, mode: 'csv' | 'raw-lines' | 'xlsx' }) {
        const bot = await this.botsService.getBot(params.bot_id);
        ...
        const trainResult = await this.localIntentsResponsesStorageService.addRawData(_dataInput, { bot_id: bot.id, currentUserId: null, project_id: params.project_id });
    ...
    private compilePrompt(prompt: IMessage[], prefix: string, config: any) {
        ...
        for (const chunk of promptReversed) {
            i++;
            const newPart = "" + config._stopString + "\n\n" + (chunk.type === MessageTypes.AI_MESSAGE ? config._assistantLabel + ':' : (chunk.type === MessageTypes.USER_MESSAGE ? config._userLabel + ':' : '')) + "\n" + chunk.message;
            if ((newPart + result).length + fullPrefix.length >= config.maxPromptLen) {
                break;
            }

            if (chunk.type === MessageTypes.AI_MESSAGE || chunk.type === MessageTypes.SYSTEM_MESSAGE) {
                if (i < promptReversed.length - 1) {
                    const nextChunk = promptReversed[i + 1];
                    const nextNewPart = "" + config._stopString + "\n\n" + (nextChunk.type === MessageTypes.AI_MESSAGE ? config._assistantLabel + ':' : (nextChunk.type === MessageTypes.USER_MESSAGE ? config._userLabel + ':' : '')) + "\n" + nextChunk.message;
                    if ((newPart + nextNewPart + result).length + fullPrefix.length >= config.maxPromptLen) {
                        break;
                    }
                }
            }
            ...
            if (chunk.type === MessageTypes.USER_MESSAGE) {
                if (i < promptReversed.length - 1) {
                    const nextChunk = promptReversed[i + 1];
                    if (nextChunk.type === MessageTypes.AI_MESSAGE || nextChunk.type === MessageTypes.SYSTEM_MESSAGE) {
                        const nextNewPart = "" + config._stopString + "\n\n" + (nextChunk.type === MessageTypes.AI_MESSAGE || nextChunk.type === MessageTypes.SYSTEM_MESSAGE ? config._assistantLabel + ':' : (nextChunk.type === MessageTypes.USER_MESSAGE ? config._userLabel + ':' : '')) + "\n" + nextChunk.message;
                        if ((nextNewPart + result).length + fullPrefix.length >= config.maxPromptLen) {
                            break;
                        }
                    }
                }
        ...
        let compiled = result.slice(result.indexOf(stopSeq) + stopSeq.length);
    ...
    private async formatPromptForIntentsMatcher(userPrompt: string, inputs: { id: number, text: string }[], config: any) {
        let promptBody = [] as IMessage[];
        ...
        for (const input of inputs) {
            promptBody.push({
                ...
                message: input.text,
                messageId: this.newMessageId(),
                previousMessageId: promptBody.length > 0 ? promptBody[promptBody.length - 1].messageId : null,
            });
            promptBody.push({
                type: MessageTypes.AI_MESSAGE,
    ...
    public async getConversationsHistory(conversationIds: string[]): Promise<IChat[]>
    public async getConversationsHistory(projectIdentification: IProjectIdentification): Promise<IChat[]>
    public async getConversationsHistory(projectIdentification: string[]|IProjectIdentification): Promise<IChat[]> {
        let conversationIds: string[] = [];
        if (Array.isArray(projectIdentification)) {
            conversationIds = projectIdentification;
        } else {
            conversationIds = Object.keys(await this.getListOfConversationIds(projectIdentification));
        }

        return await Promise.all(conversationIds.map(c => this.getConversationHistory(c)));
    }

    public async getConversationHistory(conversationId: string): Promise<IChat> {
        const conversationsFolder = this.configService.getOrThrow<string>('CONVERSATIONS_FOLDER_PATH');
        ...
        if (typeof fileData === 'object') {
            fileData.messages = (fileData.messages || []).map(m => this.formatMessage(m));
            return fileData;
        }
    ...
    private async formatPrompt(userInput: string, conversationId: string = null, config: any) {
        const versionGPT4 = !!config?.GPT4;
        ...
        let promptPrefix = "Your name is " + config._assistantLabel + ".\n" + config._promptPrefix + "\n";

        let promptBody = (await this.getConversationHistory(conversationId)).messages;

        if (promptBody.length && promptBody[0].type === MessageTypes.SYSTEM_MESSAGE) {
    ...
    private async generateConversationMessagesSlug(messages: IMessage[]) {
        ...
        return new Date(firstMessage.createdAt).toLocaleString() + " - " + firstMessage.message.slice(0, 100) + "..."
    }

    private async fixLegacyConversationsMeta(conversation: { id: string, messages_slug?: string }) {
        let isUpdated = false;

        if (!conversation.messages_slug) {
            const fullConversationData = await this.getConversationHistory(conversation.id);
            conversation.messages_slug = await this.generateConversationMessagesSlug(fullConversationData.messages);
            if (conversation.messages_slug) {
                isUpdated = true;
            }
        }
    ...
    public async getListOfConversationIds(config: { project_id?: number, bot_id?: number }) {
        const id = `${config.bot_id ?? ''}_${config.project_id ?? ''}`;

        const conversationsMetadataFolder = this.configService.getOrThrow<string>('CONVERSATIONS_METADATA_FOLDER_PATH');
        const conversationMetadataFile = conversationsMetadataFolder + id + '.json';

        let metadataContent: {
            bot_id: number,
            project_id: number,
            conversations: {[conversationId: string]: { createdAt: string, name: string, id: string, messages_slug?: string }}
        } = existsSync(conversationMetadataFile) ? JSON.parse(readFileSync(conversationMetadataFile).toString('utf-8')) : null;
        ...
        for (const conversationId in metadataContent.conversations) {
            await this.fixLegacyConversationsMeta(metadataContent.conversations[conversationId]);
        }

        return metadataContent.conversations;
    }

    private saveConversationMetadata(conversationId: string, config: { project_id?: number, bot_id?: number, name?: string, messages_slug?: string }) {
        const id = `${config.bot_id ?? ''}_${config.project_id ?? ''}`;
        ...
        const name = config.name ?? metadataContent.conversations[conversationId]?.name ?? conversationId;
        const messages_slug = config.messages_slug ?? metadataContent.conversations[conversationId]?.messages_slug ?? null;
    ...
    private async savePrompt(promptBody: IMessage[], conversationId: string, config: { project_id?: number, bot_id?: number, name?: string }) {
        const conversationsFolder = this.configService.getOrThrow<string>('CONVERSATIONS_FOLDER_PATH');
        const conversationFile = conversationsFolder + conversationId + '.json';
        ...
        this.saveConversationMetadata(conversationId, { ...config, messages_slug: fileToSave.messages_slug });
    }
