...
export type ITrainingInput = { questions: string[], answer: string, id: number };
...
export class RasaapiService {
    ...
    public async trainAndChangeModel(input: {text_original: string, id: number} [], config: IConfig) {
        const yaml = this.createYaml(input);
        console.log(yaml);
        const modelName = await lastValueFrom(this.httpService.post(config.rasa_host_url + '/model/train', yaml, { headers: { "Content-Type": 'application/yaml' }, timeout: 600000 })
        ...
        await lastValueFrom(this.httpService.put(config.rasa_host_url + '/model', { model_file: this.modelsFolder + modelName }, { headers: { "Content-Type": 'application/json' }, timeout: 600000 }));
    ...
    private createYaml(input: {text_original: string, id: number}[]) {
        ...
        input.forEach((v) => {
            ...
            nlu.push({
                "intent": intentName,
                "examples": [v.text_original].map(v => "- " + v.replace('\n', `\\n`) + '\n').join(''),
    ...
    private async getAnswerFromRasa(user_prompt: string, _userName: string = null, config: IConfig) {
        const userName = _userName || Date.now().toString();
        return await firstValueFrom(this.httpService.post(config.rasa_host_url + '/webhooks/rest/webhook', { sender: userName, message: user_prompt })
            .pipe(catchError((error => {
                this.logger.error(error, error?.stack);
                throw error;
            })))
            .pipe(map((response) => response.data.filter(({ recipient_id }) => recipient_id === userName)?.[0]?.text || null)))
    }

    public async parseFromRasa(user_prompt: string, config: IConfig) {
        return await firstValueFrom(this.httpService.post(config.rasa_host_url + '/model/parse', { text: user_prompt })
            .pipe(catchError((error => {
                this.logger.error(error, error?.stack);
                throw error;
            })))
