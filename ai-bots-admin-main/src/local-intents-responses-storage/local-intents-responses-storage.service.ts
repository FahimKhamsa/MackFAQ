...
import {
  ...
  IUpdateLocalStorageDTO,
} from './dto/localStorage.dto';
...
interface IProjectCreatePayload extends ICreateLocalStorageDTO {
  creator_id: number;
}
...
export class LocalIntentsResponsesStorageService {
  ...
  constructor(
    ...
    @InjectModel(IntentExampleModel)
    private intentExampleModel: typeof IntentExampleModel,
  ...
  public async train(
    ...
    const trainResult = await this.addRawData(_dataInput, {
  ...
  public async getAnswer(inputText: string, config: IConfig) {
    const inputTextProcessedVariants = this.replaceApostrophesWithLongForms(
      this.processIntent(inputText, true, true),
    )
      .map((t) =>
        this.replaceLongFormsWithApostrophes(this.processIntent(t, true, true)),
      )
      .flat(2);
  ...
  public async getQuestionExamplesOrCreate(
    project_id: number,
    bot_id: number,
    intent_id: number,
    question: string,
  ...
  public async getRawData(config: IConfig) {
    ...
    const promptsExamples = await this.intentExampleModel.findAll({
      where: { intent_id: prompts.map((v) => v.id) },
    });
    ...
    promptsExamples.forEach((element) => {
      if (element.intent_id in promptsByIntentId) {
        promptsByIntentId[element.intent_id].push({
          text: element.text,
          id: element.id,
        });
      } else {
        promptsByIntentId[element.intent_id] = [
    ...
    responses.forEach((element) => {
      if (element.intent_id in responsesByIntentId) {
        responsesByIntentId[element.intent_id].push({
          text: element.text,
          id: element.id,
        });
      } else {
        responsesByIntentId[element.intent_id] = [
          { text: element.text, id: element.id },
        ];
      }
    });
  ...
  public async addRawData(
    inputs: { answer: string; questions: string[] }[],
    config: IConfig,
  ) {
    const result: {
      questions: {
    ...
    for (const input of inputs) {
      ...
      const questionIntent = await this.getQuestionOrCreate(
        input.questions[0],
        config,
      );

      for (const question of input.questions) {
        const example = await this.getQuestionExamplesOrCreate(
  ...
  public async addIntentsWithResponses(
    inputs: ITrainingInput[],
    config: IConfig,
  ) {
    const result: {
      questions: {
    ...
    for (const input of inputs) {
      const resultDetecting = await this.addIntentOrUpdate(
        input.questions,
        config,
      );
      ...
      const ids = [...Object.keys(exMap), ...resultDetecting.oldIntentsId]
        .map((v) => +v)
        .filter((value, index, array) => array.indexOf(value) === index);
      for (const intentId of ids) {
        const response = await this.addResponse(
          input.answer,
          +intentId,
          config,
        );
        ...
        result.push({
          questions: examples.map((v) => ({
            text: v.text_without_punctuation_marks,
            id: v.id,
          })),
          answer: {
            id: response.id,
  ...
  private longShortFormsReplaces(
    input: string,
    params: { mode: 'long_2_short' | 'short_2_long' },
  ) {
    ...
    let contractionsDict: { [k: string]: string } = {
      ...contractionsWithNot,
      ...contractions1,
      ...contractionsWithNot,
      ...contractionsWithPronounce,
      ...contractionsWithQuestions1,
      ...contractionsWithQuestions2,
      ...contractionsWithQuestions3,
      ...contractionsWithQuestions4,
      ...UnCommonContractions,
    };
        ...
        longFormsToReplace.forEach((f) => {
          ...
          if (_input.includes(f)) {
            replacements.forEach((replacement) => {
              ...
              keysToReplaceFound[
                `${f}_${replacement}_${checkVariants ? '1' : '0'}`
              ] = [f, replacement, checkVariants];
            });
          }
        });
      }

      for (const [toReplace, replacement, checkVariants] of Object.values(
        keysToReplaceFound,
      ) as string[][]) {
        let next = null;
        const iter = _input.matchAll(
          new RegExp('(\\s|^)' + toReplace.toLowerCase() + '(\\s|$)', 'g'),
        );
        while ((next = iter.next())) {
          ...
          const local = this.processIntent(
            localInput.substring(0, next.value.index) +
              ' ' +
              replacement +
              ' ' +
              localInput.substring(next.value.index + toReplace.length + 1),
          );
          variants.push(local);
          if (checkVariants) {
    ...
    const uniqueVariants = createVariants(input).filter(
  ...
  private replaceLongFormsWithApostrophes(input: string) {
    const vairiants = this.longShortFormsReplaces(input, {
      mode: 'long_2_short',
    });
    return vairiants;
  }
  ...
  public async addIntentOrUpdate(intentTexts: string[], config: IConfig) {
    ...
    const intentTextsProcessed = intentTexts.map((text) => [
      ...
      this.replaceApostrophesWithLongForms(this.processIntent(text, true, true))
        .map((t) =>
          this.replaceLongFormsWithApostrophes(this.processIntent(t, true)),
        )
        .flat(),
    ]);

    const intentExists = await this.intentExampleModel
      .findAll({
        where: {
          text_without_punctuation_marks: intentTextsProcessed
            .map((v) => v[1])
            .flat(),
          '$intent.bot_id$': config.bot_id,
        },
      ...
      .then((r) => {
        ...
        r.forEach((example) => {
          ...
          keyByTextGrouped[example.text_without_punctuation_marks].push(
            example,
          );
        });
        return {
    ...
    for (const intentText of intentTexts) {
      ...
      for (const processedVariant of intentsProcessed[intentText]) {
        ...
        if (
          intentExists &&
          intentProcessedWithoutPunct in intentExists.keyByTextGrouped
        ) {
          for (const ex of intentExists.keyByTextGrouped[
            intentProcessedWithoutPunct
          ]) {
          ...
          intentLocal = !chosenAtTop
            ? await this.intentModel.findByPk(newLocal)
            : intentLocal;
          if (intentLocal) {
            oldIntentsId.push(intentLocal.id);
            const exLs = intentExists.keyByTextGrouped[
              intentProcessedWithoutPunct
            ].filter((r) => r.intent_id === intentLocal.id);
            if (exLs) {
              examples.push(...exLs);
            }
          }
