...
export class ProjectsController {
  ...
  async getList(@Query('bot_id') bot_id: number) {
    return {
      list: await this.localIntentsResponsesStorageService.getListOfProjects({
        bot_id,
      }),
    };
  }
  ...
  async create(@Body() body: ICreateLocalStorageDTO, @Req() req: Request) {
    ...
    return {
      status: true,
      data: await this.localIntentsResponsesStorageService.createProject(body),
    };
  }
  ...
  public async trainModel(
    ...
    return {
      status: true,
      data: await this.localIntentsResponsesStorageService.train(
  ...
  public async getAllKnowledgeRaw(
    ...
    return {
      status: true,
      data: await this.localIntentsResponsesStorageService.getCompiledTrainData(
        bot_id,
        project_id,
      ),
    };
  ...
  async update(@Body() body: IUpdateLocalStorageDTO, @Req() req: Request) {
    ...
    return {
      status: true,
      data: await this.localIntentsResponsesStorageService.updateProject(
        +body.id,
        {
          ...body,
        },
  ...
  async delete(@Body() body, @Req() req: Request) {
    ...
    return {
      status: true,
      data: await this.localIntentsResponsesStorageService.deleteProject(
        +body.id,
        {
          ...body,
        },
